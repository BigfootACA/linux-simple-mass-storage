...
int open_blkdev_or_file(const struct stat *st, const char *name, const int oflag)
{
	...
	if (-1 < fd && S_ISBLK(st->st_mode) && blkdev_is_misaligned(fd))
		warnx("warning: %s is misaligned", name);
	return fd;
}
...
int blkdev_lock(int fd, const char *devname, const char *lockmode)
{
	...
	if (strcasecmp(lockmode, "yes") == 0 ||
		...
		warnx("unsupported lock mode: %s", lockmode);
		return -EINVAL;
	}
	...
	if (!(oper & LOCK_NB)) {
		...
		if (rc != 0 && errno == EWOULDBLOCK) {
			fprintf(stderr, "%s: %s: device already locked, waiting to get lock ... ",
	...
	if (rc != 0) {
		...
		case EWOULDBLOCK: /* LOCK_NB */
			warnx("%s: device already locked", devname);
			break;
		default:
			warn("%s: failed to get lock", devname);
		}
	} else if (msg)
		fprintf(stderr, "OK\n");
	return rc;
